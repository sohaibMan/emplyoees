...
const {id,name,joiningDate,email,birthday,phone,jobName,salary,depratmentName}=req.body.Employee;
...
let query=`
UPDATE employees
SET name = "${name}", joiningDate = "${joiningDate}",email="${email}",birthday="${birthday}",phone="${phone}",jobName="${jobName}",salary=${+salary},depratmentName="${depratmentName}"
WHERE id=${+id};
`
